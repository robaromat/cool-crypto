...
# -*- coding: utf-8 -*-
"""
cool-crypto — moteur de la strategie G75 sur Bitcoin.

Une seule decision par mois (le 1er a 06:00 UTC) :
  - Tendance   : prix du BTC au-dessus de sa moyenne mobile 10 mois ?
  - Valorisation : prix vs "juste valeur" power-law (pente 5.8, intercept recalibre chaque 1er janvier).
  - Allocation G75 :
        valorisation CHERE (> 2.5x juste valeur)        -> 0 %  (cash)
        sinon, tendance HAUSSIERE                        -> 100 % BTC
        sinon (baisse mais pas chere)                    -> 75 % BTC

Ce script reconstruit tout l'historique mensuel depuis 2015 (walk-forward, sans
information du futur), puis genere : docs/history.csv, docs/data.json, docs/index.html
"""
...
GENESIS = dt.date(2009, 1, 3)   # bloc genesis Bitcoin
SLOPE = 5.8                     # pente de la power-law (fixe)
...
START = dt.date(2015, 1, 1)     # debut de l'historique publie
...
def fetch_coinmetrics():
    """Historique profond (depuis 2010), heberge sur GitHub — fiable depuis un runner."""
    raw = _get("https://raw.githubusercontent.com/coinmetrics/data/master/csv/btc.csv", 120)
...
def fetch_recent():
    ...
    # Kraken : ~720 jours
    try:
        j = json.loads(_get("https://api.kraken.com/0/public/OHLC?pair=XBTUSD&interval=1440", 30))
    ...
    try:
        j = json.loads(_get("https://api.exchange.coinbase.com/products/BTC-USD/candles?granularity=86400", 30))
...
def calibrate_A(dates, prices, upto):
    """Intercept de la power-law : moyenne(lnP - 5.8*ln(jours)) sur tout l'historique anterieur."""
...
def build():
    ...
    def fair_value(d):
        return math.exp(A_for(d) + SLOPE * math.log(days_since(d)))
    ...
    for i, k in enumerate(mkeys):
        ...
        # etat de valorisation (toujours mis a jour, meme avant START pour etre "chaud")
        if d.year >= 2013:
            fv = fair_value(d)
            ratio = p / fv
            if ratio > BAND_HIGH:
        ...
        if i + 1 < len(mkeys):
            ...
            hodl *= (1 + r)
        rows.append(dict(
            date=str(d), price=round(p, 2), fair_value=round(fv, 2), ratio=round(ratio, 3),
            sma10=round(sma, 2) if sma else None, trend="HAUSSE" if trend_up else "BAISSE",
            valuation="chère" if val_state == "CASH" else ("bon marché" if val_state == "BUY" else val_state),
...
if __name__ == "__main__":
    ...
    with open(os.path.join(DOCS, "history.csv"), "w", newline="", encoding="utf-8") as f:
        ...
        w.writerow(["date", "prix_usd", "juste_valeur_usd", "ratio", "moyenne_mobile_10m",
                    "tendance", "valorisation", "cible_btc", "action", "mult_strategie", "mult_hodl"])
        for r in rows:
    ...
    # page HTML (generee par build_site.py si present)
    try:
